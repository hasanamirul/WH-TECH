...
<div class="card" style="max-width:500px;margin:auto">
    <h2 class="mb-4">Edit Sub Unit</h2>

    @if(session('success'))
        <div style="margin-bottom:12px;color:green">{{ session('success') }}</div>
    @endif

    <form action="{{ route('master.subunit.update', $item->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <a href="{{ route('master.subunit') }}" class="px-4 py-2 bg-gray-300 rounded ml-2">Kembali</a>
    </form>
</div>
@endsection
